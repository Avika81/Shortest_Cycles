...
def BoundDijkstra(G,s,B):
    """
    get the shortest root from s up to bound B.
    returns the distances (any distance above B is inf)
...
# testing G = create_directed_weighted_graphs.create_graph(20,1/20,5,20)
# print(BoundDijkstra(G,0,t))

def Cluster_out(G,S,r,d):
    """G is a graph, S is the subset of vertices, r>0"""
    sets = []
    betta = math.log(G.number_of_nodes())/r
    x = numpy.zeros(G.number_of_nodes())
    for v in S:
...
def probabilistic_cover(G,r,s):
    c = 70 #big constant for probability
    if(G.nodes == []): return []
    s_out,s_in = estimate_balls(G,c*r,1/s)
    S_out = []
    for v in G.nodes():
        if s_out[v]>3/4 and s_in[v] > 3/4:
            if(intersects(out_ball(u,c*r),in_ball(u,c*r)<1/4 *G.number_of_nodes()):
               return  -1 # = Failure
            r = 
